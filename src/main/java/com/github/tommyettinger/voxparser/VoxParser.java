...
public class VoxParser {
    ...
    public static VoxModel readVox(LittleEndianDataInputStream stream) {
        ...
        byte[][][] voxelData = null;
        IntMap<ShapeModel> shapes = new IntMap<>(8);
                ...
                while (stream.available() > 0) {
                    ...
                    if (chunkName.equals("SIZE")) {
                        ...
                        for (int i = 0; i < dictSize; i++) {
                            ...
                            VoxMaterial vm;
                            if ((vm = lastMaterials.get(materialID, null)) == null) {
        ...
        try {
            ...
            if(materials != null && materials.notEmpty()) {
                ...
                for(IntMap.Entry<VoxMaterial> ent : materials) {
                    ...
                    dos.writeBytes(term);
                    IntFloatMap.Keys it = ent.value.traits.keys();
                    for(int k; it.hasNext;) {
                        k = it.next();
                        if(k > 9) continue;
                        VoxMaterial.MaterialTrait mt = VoxMaterial.ALL_TRAITS[k];
                        float v = ent.value.traits.get(k, 0f);
                        term = mt.name();
                        writeInt(dos, term.length());
                        dos.writeBytes(term);
