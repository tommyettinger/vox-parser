...
//                        writeSlope(result, x, y, z, slope, (byte) neighbors[bestIndex]);
    ...
    private static int isSurface(byte[][][] voxels, int x, int y, int z) {
        ...
        if(x < 0 || y < 0 || z < 0 ||
                ...
                voxels[x][y][z] == 0)
            return 0;
        if(x <= 0 || (v = voxels[x-1][y][z] & 255) == 0 || VoxParser.lastMaterials.get(v, VoxMaterial.DEFAULT_MATERIAL).getTrait(VoxMaterial.MaterialTrait._alpha) >= 1f) return 1;
        if(y <= 0 || (v = voxels[x][y-1][z] & 255) == 0 || VoxParser.lastMaterials.get(v, VoxMaterial.DEFAULT_MATERIAL).getTrait(VoxMaterial.MaterialTrait._alpha) >= 1f) return 2;
        if(z <= 0 || (v = voxels[x][y][z-1] & 255) == 0 || VoxParser.lastMaterials.get(v, VoxMaterial.DEFAULT_MATERIAL).getTrait(VoxMaterial.MaterialTrait._alpha) >= 1f) return 3;
        if(x >= voxels.length - 1 || (v = voxels[x+1][y][z] & 255) == 0 || VoxParser.lastMaterials.get(v, VoxMaterial.DEFAULT_MATERIAL).getTrait(VoxMaterial.MaterialTrait._alpha) >= 1f) return 4;
        if(y >= voxels[x].length - 1 || (v = voxels[x][y+1][z] & 255) == 0 || VoxParser.lastMaterials.get(v, VoxMaterial.DEFAULT_MATERIAL).getTrait(VoxMaterial.MaterialTrait._alpha) >= 1f) return 5;
        if(z >= voxels[x][y].length - 1 || (v = voxels[x][y][z+1] & 255) == 0 || VoxParser.lastMaterials.get(v, VoxMaterial.DEFAULT_MATERIAL).getTrait(VoxMaterial.MaterialTrait._alpha) >= 1f) return 6;
        return -1;
    }
